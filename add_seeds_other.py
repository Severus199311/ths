#coding=utf-8
#由add_seeds_json扩展而来，兼容json（get请求）、js拼接、xml形式的种子录入。不能用于post请求的种子的录入，这是因为：1.post种子的url一般重复，需要额外区分；2.curlRawText文本太长，不适合存于excel，并且send_keys太慢
...
class AddSeedsOther():
	...
	def __init__(self):
		...
		self.notice = '输入url部分和输入jsonPathIterableUrl部分是临时改的，一定要检查是否需要。'

		self.workbook_title = 'seeds_other.xls' #待录入种子所在的文件
		...
		self.if_already_exist_column_order = 16 #if_already_exist在第17列，所以是16
	...
	#打开种子录入页面
	def open_page(self):
		self.browser.get('http://flashcms.10jqka.com.cn/default/index/index')
		self.wait.until(EC.alert_is_present())
		self.browser.switch_to.alert.accept()
		self.wait.until(EC.alert_is_present())
		self.browser.switch_to.alert.accept()
		#self.wait.until(EC.presence_of_element_located((By.ID, '_M7')))
		self.browser.get('http://flashcms.10jqka.com.cn/seedmanager/html/index.html#/seed_edit')
	...
	def fetch_seed_undone(self):
		...
		if not seed_dict.get('if_successful'):
			if seed_dict.get('jsonPathIterable'):
				return seed_dict
	...
	def add_seed(self, seed_dict):
		...
		#下面这部分是临时改的，一定要检查是否需要
		url.clear()
		if seed_dict.get('json_url'):
			url.send_keys(seed_dict.get('json_url'))
		else:
			url.send_keys(seed_dict.get('url'))
		...
		#点击”我是开发者“
		i_am_developer = self.wait_and_get_element('clickable', '#pane-web > div > form > div:nth-child(12) > div > div > span')
		i_am_developer.click()

		#输入jsonPathIterable五兄弟
		jsonPathIterable = self.wait_and_get_element('located', '#pane-web > div > form > div:nth-child(15) > div:nth-child(3) > div:nth-child(3) > div > div > input')
		jsonPathIterable.clear()
		jsonPathIterable.send_keys(seed_dict.get('jsonPathIterable'))

		jsonPathIterableMagic = self.wait_and_get_element('located', '#pane-web > div > form > div:nth-child(15) > div:nth-child(3) > div:nth-child(6) > div > div > input')
		jsonPathIterableMagic.clear()
		jsonPathIterableMagic.send_keys(seed_dict.get('jsonPathIterableMagic'))

		jsonPathIterableTitle = self.wait_and_get_element('located', '#pane-web > div > form > div:nth-child(15) > div:nth-child(3) > div:nth-child(7) > div > div > input')
		jsonPathIterableTitle.clear()
		jsonPathIterableTitle.send_keys(seed_dict.get('jsonPathIterableTitle'))

		jsonPathIterableUrl = self.wait_and_get_element('located', '#pane-web > div > form > div:nth-child(15) > div:nth-child(3) > div:nth-child(8) > div > div > input')
		#jsonPathIterableUrl.clear()
		#jsonPathIterableUrl.send_keys(seed_dict.get('jsonPathIterableUrl'))
		
		#下面这部分是临时改的，一定要检查是否需要
		jsonPathIterableUrl.clear()
		if '{}' not in seed_dict.get('jsonPathIterableUrl'): #表示excel中的是真的jsonPathIterableUrl，而非模板
			jsonPathIterableUrl.send_keys(seed_dict.get('jsonPathIterableUrl'))
		else: #表示excel中的是jsonPathIterableUrl的模板
			if seed_dict.get('json_url'):
				base_url = seed_dict.get('json_url')
			else:
				base_url = seed_dict.get('url')
			url = re.match('(.*)/', base_url).group(1) + 'url'
			json_path_iterable_url = seed_dict.get('jsonPathIterableUrl').format(url)
			jsonPathIterableUrl.send_keys(json_path_iterable_url)

		name = self.wait_and_get_element('located', '#pane-web > div > form > div:nth-child(15) > div:nth-child(3) > div:nth-child(11) > div > div > input')
		name.clear()
		name.send_keys(seed_dict.get('name'))
